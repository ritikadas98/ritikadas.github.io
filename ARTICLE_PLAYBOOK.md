...
There is a fourth thing, and it is a separate job: the **preview picture** that appears
when someone pastes the article's link into Slack or LinkedIn. Without it a shared link
unfurls bare. It is generated by one script and documented in
**`PREVIEW_IMAGES_PLAYBOOK.md`** — read that when the page itself is done.

### 2a. Reddit already has a card and a listing

Nothing needs inventing. The Reddit card exists on the homepage
(`data-project="reddit"`) and is already listed on `/writing/`. It is deliberately **inert** — no link, dimmed, marked "Being written" —
